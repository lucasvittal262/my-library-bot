...
QueryResult = List[Dict[str, Union[int, float]]]


@dataclass
class LexicalSearchQueryResponse:
    pass
...
@dataclass
class NearestNeighborsQueryResponse:
...
class QdrantVectorDB:
    ...
    def get_docs_by_hibrid_search(self, dense_vector_input: List[float], sparse_vector_input: SparseEmbedding, top_k:int) -> NearestNeighborsQueryResponse:
        prefetch = [
            models.Prefetch(
...
if __name__ =="__main__":
    ...
    def create_collection(client: QdrantClient, collection_name: str, dim_dense_vector: int) -> None:
    ...
    create_collection(client=client_test, collection_name=COLLECTION_NAME, dim_dense_vector=1536)
    ...
    results = vector_db.get_docs_by_hibrid_search(dense_vector_input=query_vector, sparse_vector_input=sparse_vector, top_k=3)
    pprint(results)
